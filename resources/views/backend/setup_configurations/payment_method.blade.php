        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('Emimoney')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="payment_method" value="emimoney">
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="client_id">
                            <div class="col-lg-4">
                                <label class="col-from-label">{{translate('EMIMONEY_CLIENT_ID')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <input type="text" class="form-control" name="EMIMONEY_CLIENT_ID" value="{{  env('EMIMONEY_CLIENT_ID') }}" placeholder="{{ translate('EMIMONEY_CLIENT_ID') }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="client_secret">
                            <div class="col-lg-4">
                                <label class="col-from-label">{{translate('EMIMONEY_CLIENT_SECRET')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <input type="text" class="form-control" name="EMIMONEY_CLIENT_SECRET" value="{{  env('EMIMONEY_CLIENT_SECRET') }}" placeholder="{{ translate('EMIMONEY_CLIENT_SECRET') }}" required>
                            </div>
                        </div>
