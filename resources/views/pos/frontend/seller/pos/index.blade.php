            ...
            <div class="row gutters-10">
                <div class="col-lg-5">
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-2">
                                <input class="form-control form-control-sm" type="text" name="keyword" placeholder="{{translate('Search by Product Name/Barcode')}}" onkeyup="filterProducts()">
                            </div>
                                        ...
                                        @if (Session::has('posCart'))
                                            @forelse (Session::get('posCart') as $key => $cartItem)
                                                ...
                                                <tr>
                                                    <td>
                                                        <span class="media">
                                                            <img class="mr-3" height="60" src="{{ uploaded_asset(\App\Models\Product::find($cartItem['id'])->thumbnail_img) }}" >
        ...
        function submitOrder(payment_type){
            ...
            $.post('{{ route('pos.order_place') }}',{_token:'{{ csrf_token() }}', user_id:user_id, name:name, email:email, address:address, country:country, postal_code:postal_code, phone:phone, payment_type:payment_type, shipping:shipping, discount:discount, shipping_address:address }, function(data){
                if(data == 1){
                    ...
                    AIZ.plugins.notify('info', '{{ translate('The quantity ordered is greater than the remaining stock') }}');
                }
                else{
