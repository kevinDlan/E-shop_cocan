...
class SendSMSUtility
{
    public static function sendSMS($to, $from, $text, $template_id)
    {
        ...
        elseif (OtpConfiguration::where('type', 'emisms')->first()->value == 1) {
            $opts = array('https'=>array('header' => $_SERVER['HTTP_USER_AGENT']));
            $context = stream_context_create($opts);
            $message = urlencode($text);
            $smsapiUrl = "https://app.emisms.com/sms/api?action=send-sms&api_key=".env('EMISMS_API_KEY')."&to=225".$to."&from=".env('AGOGO_SENDER_ID')."&sms=".$message;
            $reste_sms = file_get_contents($smsapiUrl,FALSE, $context);
