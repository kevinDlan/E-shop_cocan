...
class SmsUtility
{
    ...
    public static function send_msg_to_seller_for_order($phone='', $code='')
    {
        $sms_template   = SmsTemplate::where('identifier','msg_to_seller_for_order')->first();
        $sms_body       = $sms_template->sms_body;
        $sms_body       = str_replace('[[order_code]]', $code, $sms_body);
        $template_id    = $sms_template->template_id;
        try {
    ...
    public static function send_msg_to_customer_for_order($phone='', $code='')
    {
        $sms_template   = SmsTemplate::where('identifier','msg_to_customer_for_order')->first();
        $sms_body       = $sms_template->sms_body;
        $sms_body       = str_replace('[[order_code]]', $code, $sms_body);
        $template_id    = $sms_template->template_id;
        try {
